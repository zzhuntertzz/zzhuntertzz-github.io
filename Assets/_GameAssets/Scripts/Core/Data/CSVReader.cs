...
public static class CSVReader
{
    ...
    public static List<Dictionary<string, string>> ReadNew(string data)
    {
        ...
        for(var i=0;i<data.Length;i++)
        {
            ...
            switch (c)
            {
                ...
                case ',' when !inString:
                case '\n' when !inString:
                    if (row == 0) headers.Add(value.Trim(TRIM_CHARS));
                    else entries.Add(headers[col], value.Trim(TRIM_CHARS));
                    value = "";
                    if (c == '\n')
                    {
    ...
    public static object Populate(this object obj, Dictionary<string, string> row)
    {
        ...
        foreach (var field in type.GetFields())
        {
            ...
            try
            {var valueStr = row[field.Name];
                if (string.IsNullOrEmpty(valueStr)) continue;

                var fieldType = field.FieldType;
                if (fieldType.IsEnum)
                {
                    // Xử lý enum bằng cách parse string sang enum
                    if (Enum.TryParse(fieldType, valueStr, true, out object enumValue))
                    {
                        obj.SetFieldValue(field.Name, enumValue);
                    }
                }
                else if (fieldType == typeof(float))
                {
                    if (float.TryParse(valueStr, out float floatValue))
                        obj.SetFieldValue(field.Name, floatValue);
                }
                else if (fieldType == typeof(int))
                {
                    if (int.TryParse(valueStr, out int intValue))
                        obj.SetFieldValue(field.Name, intValue);
                }
                else if (fieldType == typeof(bool))
                {
                    if (bool.TryParse(valueStr, out bool boolValue))
                        obj.SetFieldValue(field.Name, boolValue);
                }
                else if (fieldType == typeof(string))
                {
                    obj.SetFieldValue(field.Name, valueStr);
                }
